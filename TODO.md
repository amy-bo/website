...
- [ ] **Forum feed.** forum.amybo.org refuses connections from the build container, so the homepage's forum threads are a hand-picked list; a build-time fetch of latest.json would keep it fresh once the site builds on Cloudflare.
